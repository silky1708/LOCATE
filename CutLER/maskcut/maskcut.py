...
# from dinov2 import get_dinov2_key_feats
...
def get_affinity_matrix(img_feats, flow_feats, alpha, tau, eps=1e-5):
    # get affinity matrix via measuring patch-wise cosine similarity
    img_feats = img_feats.view(img_feats.shape[0], -1)
    flow_feats = flow_feats.view(flow_feats.shape[0], -1)
    
    img_feats = F.normalize(img_feats, p=2, dim=0)
    flow_feats = F.normalize(flow_feats, p=2, dim=0)
    
    A = (img_feats.transpose(0,1) @ img_feats).detach().cpu().numpy()
    B = (flow_feats.transpose(0,1) @ flow_feats).detach().cpu().numpy()
...
def maskcut_forward(img_feats, flow_feats, dims, scales, init_image_size, alpha, tau=0, N=3, cpu=False, device_id=4):
    """
    ...
    for i in range(N):
        if i == 0:
            ...
            if not cpu: painting = painting.cuda(device_id)
        else:
            feats, painting = get_masked_affinity_matrix(painting, feats, current_mask, ps)

        # construct the affinity matrix
        A, D = get_affinity_matrix(img_feats, flow_feats, alpha, tau)
...
def get_dinov2_key_feats(model, x):
    feat_out = {}
    ...
    with torch.no_grad():
        ...
        feat_h, feat_w = h // model.patch_size, w // model.patch_size
#         attentions = self.model.get_last_selfattention(img)

        tokens = model.prepare_tokens_with_masks(x, masks=None)
#         bs, nb_head, nb_token = attentions.shape[0], attentions.shape[1], attentions.shape[2]
        bs, nb_head, nb_token = x.shape[0], model.num_heads, tokens.shape[1]
        out = model(x)
        ...
        # Modality selection
        feats = k[:, 1:].transpose(1, 2).reshape(bs, model.embed_dim, feat_h * feat_w)
        return feats
            
            
            
def maskcut(img_path, flow_path, backbone, patch_size, alpha, tau, N=1, fixed_size=(848,480), cpu=False, device_id=4):
    I = Image.open(img_path).convert('RGB')
    rgb_flow = Image.open(flow_path).convert('RGB')
    bipartitions, eigvecs = [], []
        
    I_resized, w, h, feat_w, feat_h = utils.resize_pil(I, patch_size)
    flow_resized, _,_,_,_ = utils.resize_pil(rgb_flow, patch_size)
    
    img_tensor = ToTensor(I_resized).unsqueeze(0)
    flow_tensor = ToTensor(flow_resized).unsqueeze(0)
    
    if not cpu:
        img_tensor = img_tensor.cuda(device_id)
        flow_tensor = flow_tensor.cuda(device_id)
        
    img_feat = backbone(img_tensor)[0]     # DINO: dim x num_tokens
    flow_feat = backbone(flow_tensor)[0]
#     img_feat = get_dinov2_key_feats(backbone, img_tensor)[0]     # DINOv2
#     flow_feat = get_dinov2_key_feats(backbone, flow_tensor)[0]
#     img_feat = backbone.encode_image(img_tensor)[0].permute(1,0)    # CLIP
#     flow_feat = backbone.encode_image(flow_tensor)[0].permute(1,0)
    # img_feat = backbone.get_intermediate_layers(img_tensor, n=1)[0][0].permute(1,0)    # SelfPatch
    # flow_feat = backbone.get_intermediate_layers(flow_tensor, n=1)[0][0].permute(1,0)

    _, bipartition, eigvec = maskcut_forward(img_feat, flow_feat, [feat_h, feat_w], [patch_size, patch_size], [h, w], alpha, tau, N=N, cpu=cpu, device_id=device_id)

    bipartitions += bipartition
    eigvecs += eigvec
    return bipartitions, eigvecs, I
